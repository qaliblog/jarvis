...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@jarvis-ai/llm"
import type { LLMClientShape } from "@jarvis-ai/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @jarvis-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @jarvis-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
