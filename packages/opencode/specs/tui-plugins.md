...
- Author package entrypoint is `@jarvis-ai/plugin/tui`.
...
- Import types from `@jarvis-ai/plugin/tui`.
- `@jarvis-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@jarvis-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@jarvis-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@jarvis-ai/plugin/tui`.
